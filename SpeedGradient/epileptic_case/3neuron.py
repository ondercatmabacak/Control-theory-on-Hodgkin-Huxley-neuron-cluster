...
Vrest1=58*log10(((PK1*Kext1)+(PNa1*Naext1)+(PCl1*Clint1))/((PK1*Kint1)+(PNa1*Naint1)+(PCl1*Clext1)))
...
Vrest2=58*log10(((PK2*Kext2)+(PNa2*Naext2)+(PCl2*Clint2))/((PK2*Kint2)+(PNa2*Naint2)+(PCl2*Clext2))) 
...
I13          = np.zeros((len(time)))
IC13         = np.zeros((len(time)))
...
I23          = np.zeros((len(time)))
I23_st       = np.zeros((len(time)))
IC23         = np.zeros((len(time)))

V3          = np.zeros((len(time))) # mV
V3_st       = np.zeros((len(time))) # mV
V3[1]       = Vrest2
...
I31          = np.zeros((len(time)))
I31_st       = np.zeros((len(time)))
IC31         = np.zeros((len(time)))
delta       = np.zeros((len(time)))
dirac       = np.zeros((len(time)))
Icontrol    = np.zeros((len(time)))
...
for i in range(1,len(time)-1):
    ...
    I13[i]=alpha*(V1[i]-Vrest1) 
    I23[i]=alpha*(V2[i]-Vrest1)
    I31[i]=alpha*(V3[i]-Vrest2)

    dirac[i]=1/(np.sqrt(pi)*d)*np.exp(-np.power((I13[i]-I23[i]),2)/np.power(d,2))
    I31_st[i]=(-r*dirac[i]*(I13[i]-I23[i])*(V2[i]-Vst[i]))/Cm 
    V3_st[i]= I31_st[i]/alpha + Vrest1
    Icontrol[i]= -r*(V3[i]-V3_st[i])
    ...
    IC13[i]=I13[i]-IK1[i]-INa1[i]-IL1[i] 

    V1[i+1]=V1[i]+dt*IC13[i]/Cm 
    ...
    IC23[i]=I23[i] + I31[i] -IK2[i]-INa2[i]-IL2[i] 
    
    V2[i+1]=V2[i]+dt*IC23[i]/Cm 
    ...
    IC31[i]=I13[i] + I23[i] + Icontrol[i]-IK3[i]-INa3[i]-IL3[i] 

    V3[i+1]=V3[i]+dt*IC31[i]/Cm 
