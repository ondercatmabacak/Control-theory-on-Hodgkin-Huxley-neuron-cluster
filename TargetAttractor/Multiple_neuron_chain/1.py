...
for i in range(1,len(time)-1): 
    ...
    V1[i+1]= V1[i] + dt*Cm*(V1_st_dot[i] -1/TAU*(V1[i] - V1_st[i])) 

    a_n2[i]=0.01*((10-V2[i]) / (np.exp((10-V2[i])/10)-1) )  
    b_n2[i]=0.125*np.exp(-V2[i]/80)  
    a_m2[i]=0.1*((25-V2[i]) /(np.exp((25-V2[i])/10)-1) )  
    b_m2[i]=4*np.exp(-V2[i]/18)  
    a_h2[i]=0.07*np.exp(-V2[i]/20)  
    ...
    a_n3[i]=0.01*((10-V3[i]) / (np.exp((10-V3[i])/10)-1) )  
    b_n3[i]=0.125*np.exp(-V3[i]/80)  
    a_m3[i]=0.1*((25-V3[i]) /(np.exp((25-V3[i])/10)-1) )  
    b_m3[i]=4*np.exp(-V3[i]/18)  
    a_h3[i]=0.07*np.exp(-V3[i]/20)  
    ...
    m3[i+1]= dt*(a_m3[i]*(1 - m3[i]) - b_m3[i]*m3[i])+PNm[i] 
    h3[i+1]= dt*(a_h3[i]*(1 - h3[i]) - b_h3[i]*h3[i])+PNh[i] 
    n3[i+1]= dt*(a_n3[i]*(1 - n3[i]) - b_n3[i]*n3[i])+PNn[i] 
    ...
    V1_dot[i]=V1_st_dot[i] -(1/TAU)*(V1[i]-V1_st[i]) 
    V2_dot[i]=V2_st_dot[i] -(1/TAU)*(V2[i]-V2_st[i]) 
    V3_dot[i]=Vst_dot[i] -(1/TAU)*(V3[i]-Vst[i]) 
     
    I3_dot[i]=alpha*(V2_dot[i]) 
    I2_dot[i]=alpha*(V1_dot[i]) 
     
    V3_dot_dot[i]=Vst_dot_dot[i] -(1/TAU)*(V3_dot[i]-Vst_dot[i]) 
    V2_dot_dot[i]=V2_st_dot_dot[i] -(1/TAU)*(V2_dot[i]-V2_st_dot[i]) 
 
    I3_st[i]= Vst_dot[i] - (1/TAU)*(V3[i]-Vst[i]) + (1/Cm)*(F3[i]) 
    I3_st_dot[i]= Vst_dot_dot[i] - (1/TAU)*(V3[i+1]-Vst_dot[i]) + (1/Cm)*(F3_dot[i])
    I3_st_dot_dot[i]= Vst_dot_dot_dot[i] - (1/TAU)*(V3_dot_dot[i]-Vst_dot_dot[i]) + (1/Cm)*(F1_dot_dot[i]) 
     
    V2_st[i]= ((V2[i+1]-V2[i])/dt) - TAU*(V2_st_dot[i] - V2_dot[i])
 V2_st[i]=I3_st[i]/alpha +Vrest
    [i]st_dot[i]=I3_st_dot[i]/alpha 
    V2_st_dot_dot[i]=I3_st_dot_dot[i]/alpha 
     
    I2_st[i]= Vst_dot[i] - (1/TAU)*(V2[i]-Vst[i]) + (1/Cm)*(F2[i]) 
    I2_st_dot[i]= Vst_dot_dot[i] - (1/TAU)*(V2[i+1]-Vst_dot[i]) + (1/Cm)*(F2_dot[i]) 
    I2_st_dot_dot[i]= Vst_dot_dot_dot[i] - (1/TAU)*(V2_dot_dot[i]-Vst_dot_dot[i]) + (1/Cm)*(F2_dot_dot[i]) 
     
    V1_st[i]=I2_st[i]/alpha +Vrest
    V1_st_dot[i]=I2_st_dot[i]/alpha 
     
    IC[i]= Vst_dot[i] - (1/TAU)*(V1[i]-V1_st[i]) + (1/Cm)*(F1[i]) 
    I1[i]=IC[i]+Ifb[i] 
...
nDELTA     = np.sum(delta)
ndelta_bar = nDELTA*dt/(time[30]-time[20])
